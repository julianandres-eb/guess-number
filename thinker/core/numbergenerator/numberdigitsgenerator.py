...
class GeneratorDigitsNumbers:
    ...
    def generateNumbers(self, digits, possibleNumbers):
        ...
        if len(possibleNumbers) == 0:
            ...
            # We return a list with all the values, that doesn't have repeated values
            return list(filter(lambda i: len(Counter(list(str(i)))) is digits[0],
                          [i for i in range(pow(10, digits[0] - 1), pow(10, digits[0]) - 1)]))

        # If possibleNumbers has already values
        else:
            # We only return a list which we remove values that have repeated values
            return list(filter(lambda i: len(Counter(list(str(i)))) is digits[0], [i for i in possibleNumbers]))
